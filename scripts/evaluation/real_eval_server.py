...
# ----------------------------- helper (ONLY used in predict_action) -----------------------------
def _ensure_TD(x: torch.Tensor) -> torch.Tensor:
    """
    Make sure state/action tensor is (T, D) where D in {6,7,14} sits at LAST dim.

    Common cases from clients:
      - (T, D) correct  -> keep
      - (D, T) wrong    -> transpose to (T, D)
      - (B, T, D)       -> keep
      - (B, D, T)       -> transpose to (B, T, D)
    """
    if not isinstance(x, torch.Tensor):
        x = torch.tensor(x)

    # do NOT force cuda here; normalizer buffers on cpu is fine
    # keep float32 for normalization arithmetic
    ...
    if x.ndim == 2:
        # want last dim be D(6/7/14)
        if x.shape[1] in (6, 7, 14):
            return x
        if x.shape[0] in (6, 7, 14):
            return x.t().contiguous()
        return x

    if x.ndim == 3:
        # want last dim be D
        if x.shape[2] in (6, 7, 14):
            return x
        if x.shape[1] in (6, 7, 14):
...
class Server:
    ...
    def predict_action(self, payload: Dict[str, Any]) -> Any:
        try:
            ...
            # -------- FIX START: make sure states/actions are (T,D) (D at last dim) --------
            states = _ensure_TD(states)
            actions = _ensure_TD(actions)
            print(f"raw states shape(after ensure): {states.shape} ...")
            print(f"raw actions shape(after ensure): {actions.shape} ...")
            # --------------------------------------------------------------------------------

            # normalize expects last dim align with stats dim (D=7 etc.)
